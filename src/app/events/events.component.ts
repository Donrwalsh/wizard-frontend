...
export class EventsComponent {
    ...
    formatEvent(event: GameEvent) {
        return `${this.formatTime(event.when)}: ${this.parseEventDescription(event.description)}`;
    }
    ...
    parseEventDescription(description: string) {
        return description
            .replace("{focus}", "<img class='move-icon' src='/assets/moves/Focus.png' />")
            .replace("{basic mana}", "<img class='resource-icon' src='/assets/resources/Basic Mana.PNG' />")
            .replace("{basic scrolls}", "<img class='resource-icon' src='/assets/resources/Basic Scroll.png' />");
    }
